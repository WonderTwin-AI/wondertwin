...
// clock so captured webhook signatures are reproducible per seed.
...
// timestamp from clock. A nil clock falls back to time.Now() — the
// pre-CI-v2 behavior — so existing call sites continue to compile;
// determinism-sensitive callers must inject the twin's simulated
// clock explicitly.
